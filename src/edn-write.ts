...
function isStringRecord(value: unknown): value is Record<string, string> {
  ...
  if ('map' in obj || 'set' in obj || 'list' in obj || 'keyword' in obj || 'tag' in obj) {
    return false;
  }
...
export function printWire(value: unknown): string {
  ...
  if ('meta' in obj && 'value' in obj) {
    ...
    const metaPairs = meta.map(([key, entryValue]) => `${printWire(key)} ${printWire(entryValue)}`);
...
function printExtra(extra: readonly ExtraEntry[]): string[] {
  return extra.map(([key, entryValue]) => `${printWire(key)} ${printWire(entryValue)}`);
}

function printTaggedMap(tag: string, entries: string[], baseIndent: number): string {
