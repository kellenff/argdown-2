...
| `src/edn.test.ts` | Adapt assertions to `Effect.runSync(Effect.either(...))` + `_tag` |
| `src/index.ts` | `load()` uses `Effect.either` + `Either.match` to recover `LoadResult` |
| `src/builder/soft-parse.ts` | `softParse()` uses `Effect.either` + `Either.match` to recover `SoftParseResult` |
...
git commit -m "refactor(soft-parse): consume readEdn via Effect.either + Either.match"
```
...
- `readEdn` now returns `Effect.Effect<unknown, EdnError, never>`
  ...
  unwrap via `Effect.either` + `Either.match` and preserve the
...
- ✅ `index.ts` migrated to `Effect.either` + `Either.match` → Task 5
